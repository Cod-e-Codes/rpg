...
function Interactable:draw(layer)
    ...
    elseif self.type == "eastern_path" then
        -- Eastern path - A wooden bridge extending east with perspective (top-down view)
        -- Bridge extends horizontally to the right (east), narrowing with distance
        ...
        -- Create parallelogram shape (isometric perspective, no narrowing)
        -- Far end (right) is offset vertically to create depth illusion
        local depthOffset = self.height * 0.25 -- Vertical offset for depth
        local topNearY = topY
        local topFarY = topY + depthOffset
        local bottomNearY = bottomY
        local bottomFarY = bottomY + depthOffset
        
        -- Stone/dirt foundation (parallelogram for isometric depth)
        love.graphics.setColor(0.4, 0.35, 0.25)
        local foundationPoints = {
            leftX - 4, topNearY - 8,      -- Near top (left side)
            leftX - 4, bottomNearY + 8,   -- Near bottom (left side)
            rightX + 4, bottomFarY + 8,   -- Far bottom (right side)
            rightX + 4, topFarY - 8       -- Far top (right side)
        }
        love.graphics.polygon("fill", foundationPoints)
        
        -- Main bridge surface (parallelogram extending east)
        love.graphics.setColor(0.55, 0.4, 0.25)
        local bridgePoints = {
            leftX, topNearY,           -- Near top (left/west side)
            leftX, bottomNearY,        -- Near bottom (left/west side)
            rightX, bottomFarY,        -- Far bottom (right/east side)
            rightX, topFarY            -- Far top (right/east side)
        }
        love.graphics.polygon("fill", bridgePoints)
        
        -- Wooden planks (vertical lines down the bridge, getting closer together with perspective)
        local numPlanks = 7
        for i = 1, numPlanks do
            local t = i / (numPlanks + 1)
            local plankX = leftX + (rightX - leftX) * t
            local plankTopY = topNearY + (topFarY - topNearY) * t
            local plankBottomY = bottomNearY + (bottomFarY - bottomNearY) * t
            
            -- Alternate plank colors for depth
            if i % 2 == 0 then
                love.graphics.setColor(0.5, 0.35, 0.22)
            else
                love.graphics.setColor(0.48, 0.33, 0.20)
            end
            love.graphics.setLineWidth(3)
            love.graphics.line(plankX, plankTopY, plankX, plankBottomY)
        end
        love.graphics.setLineWidth(1)
        
        -- Top side railing/support (perspective line)
        love.graphics.setColor(0.35, 0.25, 0.15)
        love.graphics.setLineWidth(6)
        love.graphics.line(leftX, topNearY + 6, rightX, topFarY + 4)
        love.graphics.setLineWidth(1)
        
        -- Bottom side railing/support (perspective line)
        love.graphics.setColor(0.35, 0.25, 0.15)
        love.graphics.setLineWidth(6)
        love.graphics.line(leftX, bottomNearY - 6, rightX, bottomFarY - 4)
        love.graphics.setLineWidth(1)
        
        -- Support posts (horizontal lines at intervals)
        for i = 0, 3 do
            local t = i / 3
            local postX = leftX + (rightX - leftX) * t
            local postTopY = topNearY + (topFarY - topNearY) * t + 6
            local postBottomY = bottomNearY + (bottomFarY - bottomNearY) * t - 6
            
            love.graphics.setColor(0.3, 0.2, 0.12)
            love.graphics.setLineWidth(4)
            -- Horizontal post
            love.graphics.line(postX - 4, postTopY, postX + 4, postTopY)
            love.graphics.line(postX - 4, postBottomY, postX + 4, postBottomY)
        end
        love.graphics.setLineWidth(1)
        
        -- Golden magical glow around the entire bridge (slightly outside the parallelogram)
        love.graphics.setColor(1.0, 0.85, 0.4, 0.15 * glowPulse)
        local glowPoints = {
            leftX - 8, topNearY - 12,
            leftX - 8, bottomNearY + 12,
            rightX + 8, bottomFarY + 12,
            rightX + 8, topFarY - 12
        }
        love.graphics.polygon("fill", glowPoints)
        
        -- Glowing lanterns at near corners (left side)
        love.graphics.setColor(1.0, 0.8, 0.3, 0.9 * glowPulse)
        love.graphics.circle("fill", leftX + 4, topNearY + 8, 5)
        love.graphics.circle("fill", leftX + 4, bottomNearY - 8, 5)
        love.graphics.setColor(1.0, 0.95, 0.6, 0.7 * glowPulse)
        love.graphics.circle("fill", leftX + 4, topNearY + 8, 3)
        love.graphics.circle("fill", leftX + 4, bottomNearY - 8, 3)
        
        -- Magical sparkles drifting along the bridge (moving horizontally to the right)
        for i = 0, 6 do
            local t = (i / 6 + time * 0.2) % 1
            local sparkleX = leftX + (rightX - leftX) * t
            local sparkleTopY = topNearY + (topFarY - topNearY) * t
            local sparkleBottomY = bottomNearY + (bottomFarY - bottomNearY) * t
            local sparkleCenterY = (sparkleTopY + sparkleBottomY) / 2
            local sparkleOffsetY = math.sin(time * 2 + i) * 10
            
            local sparkleSize = (1.8 - t * 0.5) + math.sin(time * 4 + i) * 0.3
            local sparkleAlpha = math.sin(t * math.pi) * glowPulse * 0.9
            
            love.graphics.setColor(1.0, 0.95, 0.6, sparkleAlpha)
            love.graphics.circle("fill", sparkleX, sparkleCenterY + sparkleOffsetY, sparkleSize)
        end
        
        -- Dark outline for depth (parallelogram border)
        love.graphics.setColor(0.2, 0.15, 0.1)
        love.graphics.setLineWidth(3)
        love.graphics.polygon("line", bridgePoints)
        love.graphics.setLineWidth(1)
    end
